...
// 1. 开始校验
function toNext() {
  console.log('next----xiayib');
  
  
  // 账号未注册，跳转到注册页补充信息
  uni.navigateTo({ url: `/pages/login/signup` })
  if (authStore.phone && data.inputCode) {
    uni.showLoading({ title: '请稍后', mask: true })
    
    authStore.getVerifyCode({phone: authStore.phone, captcha:data.inputCode}).then((res: any) => {
      uni.hideLoading()
      if (res.code === 200 && res.data) {
        authStore.code = data.inputCode
        // 验证成功
        toLogin()
      } else {
        uni.showToast({
...
// 重新发送验证码
function reSendCode() {
  if (authStore.phone && data.nextTime === 0) {
    uni.showLoading({ title: '发送中' })
    authStore
      .getSendCode({phone: authStore.phone})
      .then((res: any) => {
        if (!res.data) {
          uni.showToast({
            title: '发送失败',
...
<template>
  ...
  <!-- ↓ 自定义导航 -->
  <set-nav-bar :title="'手机号登陆'" :back="true" :filter="false" :bg="true" />
  ...
  <view class="login-at-phone">
    <view class="login-at-phone__main">
      ...
      <view class="login-at-phone__main-title-p">
        <text>已经发送至 {{ authStore.phone }}</text>
